 ...
 * ClassName CateShoujiAdapter
 * Created by yuxin.
 * Created time 13-12-2016 15:13.
 * Describe : 手机 fragment 没有添加广告控件
 * History:
 * Version   1.0.
 *
 *==============================================================================
 */
public class CateShoujiAdapter extends RecyclerView.Adapter<CateShoujiAdapter.ViewHolder> {

    private static final String TAG = CateShoujiAdapter.class.getSimpleName();

    private Context context;
    //里面的数据
    private List<NewslistBean> newslistBeen = new ArrayList<>();

    public CateShoujiAdapter(Context context) {
    ...
    @Override
    public CateShoujiAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
     //返回ViewHolder
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_adapter_item_two, parent, false);
        CateShoujiAdapter.ViewHolder vh = new CateShoujiAdapter.ViewHolder(view);
        return vh;
    }

    @Override
    public void onBindViewHolder(CateShoujiAdapter.ViewHolder holder, final int position) {
      //图片加载失败
